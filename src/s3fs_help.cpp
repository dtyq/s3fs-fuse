...
static constexpr char help_string[] =
    ...
    "\n"
    "   max_file_size (default=\"-1\" which means no limit)\n"
    "      - Set the maximum file size limit in bytes. When a write operation\n"
    "        would exceed this limit, S3FS returns EFBIG (File too large)\n"
    "        error. Use -1 to disable the limit (default behavior).\n"
    "        Examples:\n"
    "          -o max_file_size=104857600    (100MB limit)\n"
    "          -o max_file_size=1073741824   (1GB limit)\n"
    "          -o max_file_size=-1           (no limit, default)\n"
    "\n"
    ...
    "        will be used.\n"
    "\n"
    "   http_notify_url (default=\"\" which means disabled)\n"
    "        Enable HTTP notifications for file operations (CREATE, UPDATE, DELETE).\n"
    "        When specified, s3fs will send JSON formatted notifications to the given\n"
    "        URL for all file/directory operations. The notification includes operation\n"
    "        type, file path, file size, timestamp and mount point information.\n"
    "        Notifications are sent asynchronously and failures won't affect file\n"
    "        operations.\n"
    "\n"
    "   http_notify_timeout (default=5000)\n"
    "        Timeout in milliseconds for HTTP notification requests.\n"
    "        This option is only effective when http_notify_url is specified.\n"
    "\n"
    "   http_notify_exclude_path (default=\"\" which means no exclusions)\n"
    "        Specify directory paths to exclude from HTTP notifications.\n"
    "        Files and directories under these paths will not trigger notifications.\n"
    "        Multiple paths can be specified using colon (:) as separator.\n"
    "        Paths should be relative to the mount point and start with /.\n"
    "        This option is only effective when http_notify_url is specified.\n"
    "        Examples:\n"
    "          -o http_notify_exclude_path=/tmp\n"
    "          -o http_notify_exclude_path=/tmp:/cache:/var/log\n"
    "\n"
