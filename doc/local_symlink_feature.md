...
#### 3.2.1 创建内部函数 `create_symlink_internal`
...
| 文件 | 修改内容 |
|------|----------|
| `src/s3fs.cpp` | 添加 `create_symlink_internal`、修改 `s3fs_mkdir`、添加选项解析 |
...
static int s3fs_mkdir(const char* _path, mode_t mode)
{
    ...
    if(is_local_symlink && LocalSymlinkManager::instance().should_create_local_symlink(path)){
        ...
        result = create_symlink_internal(local_target.c_str(), path, 
...
创建 `create_symlink_internal` 函数，直接通过 S3 API 创建软链接对象，绕过 VFS 层，避免死锁。
